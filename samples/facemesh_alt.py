...
def landmarksDetection(img, results, draw=False):
    ...
    if draw :
        [cv.circle(img, p, 1, utils.WHITE, -1) for p in mesh_coord]
...
with map_face_mesh.FaceMesh(min_detection_confidence =0.5, min_tracking_confidence=0.5) as face_mesh:
    ...
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            frame =utils.fillPolyTrans(frame, [mesh_coords[p] for p in FACE_OVAL], utils.WHITE, opacity=0.1)
            frame =utils.fillPolyTrans(frame, [mesh_coords[p] for p in LEFT_EYE], utils.GREEN, opacity=1)
            frame =utils.fillPolyTrans(frame, [mesh_coords[p] for p in RIGHT_EYE], utils.GREEN, opacity=1)
            ...
            frame =utils.fillPolyTrans(frame, [mesh_coords[p] for p in LIPS], utils.GREEN, opacity=0.3 )
